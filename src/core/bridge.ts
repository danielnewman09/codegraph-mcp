 ...
 *
 * Spawns ``<python> <bridgePath>``, speaks newline-delimited JSON on its
 * stdin/stdout, and routes correlation-ids back to awaiting callers.
 * Stderr from the child is forwarded to stderr so diagnostics are visible
 * without corrupting the framing channel.
 *
 * Host-neutral: no Pi or MCP imports.
 */
...
// ── CodegraphBridge: stdio JSON-RPC client ────────────────────────────────
